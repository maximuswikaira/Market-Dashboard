#!/usr/bin/env python3
"""
Fetches US market indices + top movers and renders a static HTML dashboard.

Data source: Yahoo Finance's public (undocumented) endpoints. No API key needed,
but these endpoints can occasionally change shape or rate-limit — see README
for a fallback plan if that happens.
...
def render_html(indices, sections):
    ...
    sections_html = "".join(
        f"<section><h2>{title}</h2>{table(rows)}</section>"
...
def main():
    ...
    html = render_html(indices, sections)
